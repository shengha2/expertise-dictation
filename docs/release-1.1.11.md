...
The installer preview passed 25 checks for its real mounted contents, matching app/volume logo, portable native background bookmark, non-overlapping layout and Retina image sizes. Finder visual inspection caught and corrected an initially unresolved background; the working artwork was then checked in Finder. Update preparation subsequently rejected an unpublished, notarized image because hiding the app extension added FinderInfo to its signed bundle. That decoration was removed. Packaging now verifies the app's strict signature after all installer decoration, and the mounted-image validator adds the same check; the rejected image proves that this check fails on the defect. Final packaging separately enforces Developer ID signatures, Apple notarization and Gatekeeper acceptance.
